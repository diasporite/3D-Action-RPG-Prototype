using System.Collections;
using System.Collections.Generic;
using UnityEngine;

namespace RPG_Project
{
    [System.Serializable]
    public class Statf
    {
        [SerializeField] protected float statValue;
        [SerializeField] protected float currentStatValue;

        protected readonly List<StatModifier> statModifiers = new List<StatModifier>();

        protected float valueCap = 255;

        // Look up 'dirty flag'
        protected bool valueChanged = false;

        public float StatValue
        {
            get => statValue;

            set
            {
                statValue = value;

                if (statValue < 1) statValue = 1;
                if (statValue > valueCap) statValue = valueCap;
            }
        }

        public float _currentStatValue => currentStatValue;

        #region Constructors
        public Statf(float statValue)
        {
            InitValues(statValue);
        }

        public Statf(float statValue, float statCap)
        {
            valueCap = statCap;

            InitValues(statValue);
        }

        protected virtual void InitValues(float value)
        {
            //valueCap = GameManager.instance._statCap;

            StatValue = value;
            currentStatValue = value;
        }
        #endregion

        float CurrentValue()
        {
            float value = statValue;

            foreach (var mod in statModifiers)
            {
                switch (mod._modifierType)
                {
                    case StatModifierType.Additive:
                        value += mod._valueChange;
                        break;
                    case StatModifierType.Subtractive:
                        value -= mod._valueChange;
                        break;

                    case StatModifierType.PercentIncrease:
                        value += 0.01f * mod._valueChange * statValue;
                        break;
                    case StatModifierType.PercentDecrease:
                        value -= 0.01f * mod._valueChange * statValue;
                        break;

                    case StatModifierType.PercentIncreaseCompound:
                        value *= (1 + 0.01f * mod._valueChange);
                        break;
                    case StatModifierType.PercentDecreaseCompound:
                        value *= (1 - 0.01f * mod._valueChange);
                        break;

                    default:
                        break;
                }
            }

            if (value < 1) value = 1;
            if (value > valueCap) value = valueCap;

            return value;
        }

        public void AddModifier(StatModifier modifier)
        {
            statModifiers.Add(modifier);
            valueChanged = true;
            currentStatValue = CurrentValue();
        }

        public void RemoveModifier(StatModifier modifier)
        {
            if (statModifiers.Contains(modifier))
            {
                statModifiers.Remove(modifier);
                valueChanged = true;
                currentStatValue = CurrentValue();
            }
        }

        public void ClearModifier()
        {
            statModifiers.Clear();
            valueChanged = true;
            currentStatValue = CurrentValue();
        }
    }
}